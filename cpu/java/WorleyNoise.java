...
/* Ported from C to Java by Stefan Gustavson (stegu@itn.liu.se). */
...
public final class WorleyNoise {
	...
	private static final int Poisson_count[] = { 4, 3, 1, 1, 1, 2, 4, 2, 2, 2,
			5, 1, 0, 2, 1, 2, 2, 0, 4, 3, 2, 1, 2, 1, 3, 2, 2, 4, 2, 2, 5, 1,
			2, 3, 2, 2, 2, 2, 2, 3, 2, 4, 2, 5, 3, 2, 2, 2, 5, 3, 3, 5, 2, 1,
			3, 3, 4, 4, 2, 3, 0, 4, 2, 2, 2, 1, 3, 2, 2, 2, 3, 3, 3, 1, 2, 0,
			2, 1, 1, 2, 2, 2, 2, 5, 3, 2, 3, 2, 3, 2, 2, 1, 0, 2, 1, 1, 2, 1,
			2, 2, 1, 3, 4, 2, 2, 2, 5, 4, 2, 4, 2, 2, 5, 4, 3, 2, 2, 5, 4, 3,
			3, 3, 5, 2, 2, 2, 2, 2, 3, 1, 1, 4, 2, 1, 3, 3, 4, 3, 2, 4, 3, 3,
			3, 4, 5, 1, 4, 2, 4, 3, 1, 2, 3, 5, 3, 2, 1, 3, 1, 3, 3, 3, 2, 3,
			1, 5, 5, 4, 2, 2, 4, 1, 3, 4, 1, 5, 3, 3, 5, 3, 4, 3, 2, 2, 1, 1,
			1, 1, 1, 2, 4, 5, 4, 5, 4, 2, 1, 5, 1, 1, 2, 3, 3, 3, 2, 5, 2, 3,
			3, 2, 0, 2, 1, 1, 4, 2, 1, 3, 2, 1, 2, 2, 3, 2, 5, 5, 3, 4, 5, 5,
			2, 4, 4, 5, 3, 2, 2, 2, 1, 4, 2, 3, 3, 4, 2, 5, 4, 2, 4, 2, 2, 2,
			4, 5, 3, 2 };
	...
	private static void AddSamples(int xi, int yi, int zi, int max_order,
			double at[], double F[], double delta[][], int ID[]) {
		double dx, dy, dz, fx, fy, fz, d2;
		int count, i, j, index;
		int seed, this_id;

		/*
		 * Each cube has a random number seed based on the cube's ID number. The
		 * seed might be better if it were a nonlinear hash like Perlin uses for
		 * noise but we do very well with this faster simple one. Our LCG uses
		 * Knuth-approved constants for maximal periods.
		 */
		seed = 702395077 * xi + 915488749 * yi + 2120969693 * zi;

		/* How many feature points are in this cube? */
		count = Poisson_count[(seed >> 24) & 0xFF]; /*
													 * 256 element lookup table.
													 * Use MSB
													 */

		seed = 1402024253 * seed + 586950981; /*
											 * churn the seed with good Knuth
											 * LCG
											 */

		for (j = 0; j < count; j++) /*
									 * test and insert each point into our
									 * solution
									 */
		{
			this_id = seed;
			...
			/*
			 * Distance computation! Lots of interesting variations are possible
			 * here! Biased "stretched" A*dx*dx+B*dy*dy+C*dz*dz Manhattan
			 * distance fabs(dx)+fabs(dy)+fabs(dz) Radial Manhattan:
			 * A*fabs(dR)+B*fabs(dTheta)+C*dz Superquadratic: pow(fabs(dx), A) +
			 * pow(fabs(dy), B) + pow(fabs(dz),C)
			 * 
			 ...
			 * them! [Alternatively, change the search algorithm for your
			 * special cases.]
			 */
			...
			if (d2 < F[max_order - 1]) /*
										 * Is this point close enough to
										 * remember?
										 */
			{
				/*
				 * Insert the information into the output arrays if it's close
				 * enough. We use an insertion sort. No need for a binary search
				 * to find the appropriate index.. usually we're dealing with
				 * order 2,3,4 so we can just go through the list. If you were
				 * computing order 50 (wow!!) you could get a speedup with a
				 * binary search in the sorted F[] list.
				 */
				...
				/*
				 * Bump down more distant information to make room for this new
				 * point.
				 */
