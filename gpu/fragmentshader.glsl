...
void main() {
	vec2 coord = vec2(st.s * 10., st.t * 10.) + 0.2 * vec2(snoise(vec3(st.s, st.t, 2)));

	/* vec3 groundcolor = texture(tex,coord).rgb; */
	vec3 brickcolor = vec3(0.8, 0.8, 0.8);
	float alpha = texture(tex, st+vec2(-0.02*time, 0.0)).a;

	vec3 cloudcolor = vec3(1.0, 1.0, 1.0);
	
	vec3 diffusecolor = mix(groundcolor, groundcolor, alpha);
	
	vec3 nNormal = normalize(interpolatedNormal);
	
	float diffuse = max(0.0, nNormal.z);

	color = vec4(diffusecolor * diffuse, 1.0);
}
